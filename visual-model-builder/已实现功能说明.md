...
项目现在已经不仅能可视化搭建 CNN 模型，还能完成最小训练图构建、训练代码生成、训练执行、训练结果展示和结果保存。
...
8. `Run Training` 可以返回 `loss` 和 `accuracy`
9. `Curves` 视图能显示曲线或点
10. `Save Results` 能导出 HTML 报告
11. 后端 `training_runs` 目录能看到权重和 JSON 摘要
12. `Ctrl/Cmd + C / V / Z / S` 正常工作
13. 第一个节点添加后可立即撤回
14. 粘贴节点不会与已有节点完全重叠
