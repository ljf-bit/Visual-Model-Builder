"""Validation rules for model graphs and training graphs."""
...
def _validate_common_graph(
    ...
    for node in ir_graph.nodes:
        ...
        if node.op == "Input":
            ...
            upstream_model_refs = [ref for ref in node.input_refs if node_map.get(ref) and node_map[ref].category == "model"]
            downstream_model_refs = [ref for ref in node.output_refs if node_map.get(ref) and node_map[ref].category == "model"]
            if len(upstream_model_refs) > 1:
                _add_node_error(node_errors, node.node_id, "The current model graph supports only a single upstream model connection.")
            if len(downstream_model_refs) > 1:
